...
export async function getSessionSpend(api: string): Promise<number> {
  const [row] = await db
    ...
    .where(
      sql`${schema.apiSpend.api} = ${api} AND ${schema.apiSpend.createdAt} > now() - interval '1 day'`
    );
  return row.total;
}
...
export async function getMonthlySpend(api: string): Promise<number> {
  const [row] = await db
    ...
    .from(schema.apiSpend)
    .where(
      sql`${schema.apiSpend.api} = ${api} AND ${schema.apiSpend.createdAt} > date_trunc('month', now())`
    );
  return row.total;
}
...
export async function getCachedMedia(hash: string): Promise<string | null> {
...
export async function setCachedMedia(
  ...
  await db
    .insert(schema.mediaCache)
    .values({ hash, api, filePath, prompt, model, config })
    .onConflictDoUpdate({
...
export async function setSessionPermission(api: string, decision: string): Promise<void> {
  await db
    .insert(schema.permissionSessions)
    .values({ api, decision })
    .onConflictDoUpdate({
...
export async function getAgentUsageSummary(projectId?: string): Promise<{
  ...
  const empty = { totalInputTokens: 0, totalOutputTokens: 0, totalCostUsd: 0, totalApiCalls: 0, totalToolCalls: 0, byAgent: {} as Record<string, { inputTokens: number; outputTokens: number; costUsd: number; count: number }> }

  try {
    const whereClause = projectId
      ? sql`WHERE ${schema.agentUsage.projectId} = ${projectId}`
      : sql``;
