...
export function MessageBubble({ msg, isStreaming, activeToolName, agentPhase, onRate, onPermission, onEdit, onRetry }: MessageBubbleProps) {
  ...
  const isError = typeof msg.content === 'string' &&
    (msg.content.startsWith('Error:') || msg.content.startsWith('Failed to connect'))
          ...
          {isEditing ? (
            <div className="rounded-xl border border-[var(--color-accent)]/50 bg-[var(--agent-chat-user-surface)] overflow-hidden">
              ...
              <div className="flex items-center gap-2 px-3 pb-2">
                <button onClick={() => { onEdit?.(msg.id, editValue); setIsEditing(false) }}
            ...
            <div className="rounded-xl border border-[var(--color-border)] bg-[var(--agent-chat-user-surface)] px-3.5 py-2.5 relative">
              <button
                onClick={() => { setEditValue(textContent); setIsEditing(true) }}
              ...
              <ReactMarkdown
                components={{
                  code({ className, children, ...props }) {
                    ...
                    if (isInline) {
                      return (
                        <code className="bg-[var(--color-border)]/50 px-1 py-0.5 rounded text-[12px] font-mono text-[var(--color-text-primary)]" {...props}>
                          {children}
                        </code>
                      )
                    }
                    return (
                      <SyntaxHighlighter
                        ...
                        customStyle={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', margin: '8px 0' }}
                      >
