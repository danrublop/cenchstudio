...
/** Default Unsplash / stock tiles (React Bits). */
export const DEFAULT_DOME_IMAGES: { src: string; alt: string }[] = [
  {
...
function getDataNumber(el: HTMLElement, name: string, fallback: number): number {
  ...
  const attr = ds[name] ?? el.getAttribute(`data-${name}`)
...
function buildItems(pool: DomeGalleryImageItem[], seg: number): TileCoord[] {
...
export type DomeGalleryProps = {
  ...
  grayscale?: boolean
  /** Caps radius relative to viewport height; higher = wider dome on wide/short screens */
  heightGuardFactor?: number
}

export default function DomeGallery({
  images = DEFAULT_DOME_IMAGES,
  fit = 0.5,
  fitBasis = 'auto',
  minRadius = 600,
  maxRadius = Infinity,
          ...
          <div ref={sphereRef} className="sphere">
            {items.map((it, i) => (
              ...
              >
                ...
                >
                  {/* eslint-disable-next-line @next/next/no-img-element -- DomeGallery tiles use dynamic remote URLs */}
